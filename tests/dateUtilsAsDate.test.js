...
(function runTests() {
  ...
  const altTimestamp = { _seconds: Math.floor(ms / 1000), _nanoseconds: 0 };
  const fromAltTimestamp = asDate(altTimestamp);
  assert(fromAltTimestamp instanceof Date, "Alternative timestamp fields should be supported");
  assert.strictEqual(fromAltTimestamp.getTime(), ms, "_seconds/_nanoseconds should normalize correctly");
  ...
  assert.strictEqual(asDate(sentinelTimestamp), null, "Server timestamp sentinels should return null");

  const epochFallback = new Date(0);
  assert.strictEqual(asDate(epochFallback), null, "Epoch fallback dates should be ignored");
