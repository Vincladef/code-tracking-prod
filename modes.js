...
try {
  if (typeof window !== "undefined" && window.HistoryStore) {
    ...
    HistoryStore = require("./utils/history-store.js");
  }
} catch (_) {
  HistoryStore = null;
}
...
const ensureHistoryStoreContext = (ctx = null) => {
  ...
  const appCtx = ctx || (typeof window !== "undefined" ? window.AppCtx : null) || {};
  const db = appCtx?.db || null;
  const uid = appCtx?.user?.uid || null;
  const dayKeyFromDate = typeof Schema?.dayKeyFromDate === "function" ? Schema.dayKeyFromDate : null;
  HistoryStore.configure({ db, uid, dayKeyFromDate });
};

const dispatchHistoryUpdateEvent = (detail = {}) => {
...
const historyStoreUpsert = (consigneId, entry) => {
  if (!consigneId || !HistoryStore || typeof HistoryStore.upsert !== "function") {
    return null;
  }
...
const historyStoreRemove = (consigneId, dayKey) => {
  if (!consigneId || !HistoryStore || typeof HistoryStore.remove !== "function") {
    return false;
  }
  ensureHistoryStoreContext();
...
const inferConsigneTypeFromRow = (row, consigneId) => {
  if (!row) return "short";
  if (row.querySelector?.("[data-checklist-root]")) {
    return "checklist";
  }
  const safeId = escapeSelectorToken(consigneId);
  const field = row.querySelector?.(`[name$=":${safeId}"]`);
  ...
  if (clearBtn) {
    ...
    clearBtn.addEventListener("click", async (event) => {
      ...
      try {
        ...
        try {
          historyStoreRemove(consigne.id, dayKeyToClear);
        } catch (_) {
          try { historyStoreInvalidate(consigne.id); } catch (_) {}
        }
        try {
          bufferHistoryEntry(consigne.id, null, dayKeyToClear);
        } catch (_) {}
        ...
        for (const childState of baseChildStates) {
          ...
          try {
            historyStoreRemove(childConsigneId, resolvedDayKey);
          } catch (_) {
            try { historyStoreInvalidate(childConsigneId); } catch (_) {}
          }
          try {
            bufferHistoryEntry(childConsigneId, null, resolvedDayKey);
          } catch (_) {}
          applyDailyPrefillUpdate(childState.consigne.id, resolvedDayKey, "", { remove: true, silent: false });
          dispatchHistoryUpdateEvent({
            consigneId: childState.consigne.id,
            dayKey: resolvedDayKey,
            entry: null,
            silent: false,
          });
  ...
  form?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const writeHistoryId = (() => {
        ...
        const todayKey = typeof Schema?.todayKey === "function" ? Schema.todayKey() : "";
      ...
      await runWithAutoSaveSuppressed(consigne.id, runDayKey, async () => {
        ...
        const effectiveEntryForRow = storeRecord || (parentHasValue ? {
          ...
          normalizedDayKey: dispatchDayKey,
          value: rawValue,
          historyId: effectiveHistoryId,
          responseId: resolvedResponseId,
        } : null);

        applyTransientRowUpdate(consigne, dispatchDayKey, effectiveEntryForRow, row);
        if (typeof refreshConsigneTimelineWithRows === "function") {
          try {
            let previewEntries = Array.isArray(timelinePreviewEntries)
              ...
              : [];
            const normalizedTargetKey = normalizeHistoryDayKey(dispatchDayKey);
        ...
        for (const { state, value, hasValue } of childResults) {
          ...
          const childDispatchDayKey = dispatchDayKey;
          const normalizedChildTimelineKey = normalizeHistoryDayKey(childDispatchDayKey);
          ...
          if (hasValue) {
            ...
            if (typeof syncDailyRowFromHistory === "function") {
              try {
                syncDailyRowFromHistory(childConsigneId, childDispatchDayKey, {
                  entry: childRecord,
            ...
            applyTransientRowUpdate(state?.consigne || { id: childConsigneId }, childDispatchDayKey, childRecord, state?.row || null);
          } else {
            try {
            ...
            applyTransientRowUpdate(state?.consigne || { id: childConsigneId }, childDispatchDayKey, null, state?.row || null);
          }
        }
      });
        if (storeRecord) {
          applyDailyPrefillUpdate(consigne.id, dispatchDayKey, rawValue, { entry: storeRecord, silent: false });
          dispatchHistoryUpdateEvent({
            consigneId: consigne.id,
            dayKey: dispatchDayKey,
            entry: storeRecord,
          ...
          if (typeof syncDailyRowFromHistory === "function") {
            try {
              syncDailyRowFromHistory(consigne.id, dispatchDayKey, {
                entry: storeRecord,
                fallbackDayKey: resolvedDayKey,
                silent: false,
                transient: false,
          ...
          applyDailyPrefillUpdate(consigne.id, dispatchDayKey, "", { remove: true, silent: false });
          dispatchHistoryUpdateEvent({
            consigneId: consigne.id,
            dayKey: dispatchDayKey,
            entry: null,
          ...
          applyTransientRowUpdate(consigne, dispatchDayKey, null, row);
          if (typeof refreshConsigneTimelineWithRows === "function") {
            try {
              const latestEntries = HistoryStore && typeof HistoryStore.getEntries === "function"
                ? HistoryStore.getEntries(consigne.id)
    ...
    clearBtn.addEventListener("click", async (event) => {
      ...
      try {
        ...
        for (const childState of baseChildStates) {
          ...
          try {
            historyStoreRemove(childConsigneId, dayKeyToClear);
          } catch (_) {
            try { historyStoreInvalidate(childConsigneId); } catch (_) {}
          }
          try {
            bufferHistoryEntry(childConsigneId, null, dayKeyToClear);
          } catch (_) {}
          applyDailyPrefillUpdate(childState.consigne.id, dayKeyToClear, "", { remove: true, silent: false });
          dispatchHistoryUpdateEvent({
            consigneId: childState.consigne.id,
            dayKey: dayKeyToClear,
            entry: null,
...
function updateConsigneHistoryTimeline(row, status, options = {}) {
  ...
  const requestedHistoryId = typeof options.historyId === "string" ? options.historyId.trim() : "";
  if (options && options.remove !== true && !requestedHistoryId) {
    return;
  }
...
function applyHistoryEntryToRow(consigne, dayKey, entry, options = {}) {
  if (!consigne) {
    return;
  }
  ...
  const resolveKey = (source) => {
    if (!source) return "";
    const normalized = normalizeHistoryDayKey(source);
  ...
  const entryDayKey = entry && typeof entry === "object"
    ? resolveKey(
        entry.normalizedDayKey ||
          entry.dayKey ||
          entry.dateKey ||
          entry.date ||
          (entry.metadata && (entry.metadata.dayKey || entry.metadata.dateKey)) ||
          null,
      )
    : "";
  const normalizedDayKey = resolveKey(dayKey) || entryDayKey || resolveKey(fallbackDayKey);

  let row = providedRow;
  if (!(row instanceof HTMLElement)) {
    const selector = normalizedDayKey
      ? `[data-consigne-id="${escapeSelectorToken(rawConsigneId)}"][data-day-key="${escapeSelectorToken(normalizedDayKey)}"]`
      : `[data-consigne-id="${escapeSelectorToken(rawConsigneId)}"]`;
    try {
  ...
  if (normalizedDayKey) {
    try {
      row.dataset.dayKey = normalizedDayKey;
    } catch (_) {}
  }

  const historyId = entry ? resolveHistoryDocumentId(entry, normalizedDayKey || "") : "";
  const responseId = entry ? resolveHistoryResponseId(entry, "") : "";
  try {
    if (historyId) {
      row.dataset.historyId = String(historyId);
    } else {
      delete row.dataset.historyId;
    }
  } catch (_) {}
  try {
    if (responseId) {
      row.dataset.responseId = String(responseId);
    } else {
      delete row.dataset.responseId;
    }
  } catch (_) {}

  if (row.dataset?.historyPending === "1") {
  ...
  try {
    logConsigneSnapshot("daily.row.apply", consigne, {
      row,
      extra: {
...
function attachConsigneEditor(row, consigne, options = {}) {
  ...
  const openEditor = async () => {
    const ctxRef = options.ctx || (typeof window !== "undefined" ? window.AppCtx || null : null);
    const rawDayKey = (row?.dataset?.dayKey && row.dataset.dayKey.trim())
      || (typeof window !== "undefined" && window.AppCtx && typeof window.AppCtx.dateIso === "string"
        ? window.AppCtx.dateIso
        : "");
    const normalizedDayKey = rawDayKey ? normalizeHistoryDayKey(rawDayKey) : "";
    if (!normalizedDayKey) {
      showToast("Jour introuvable pour cette consigne.");
        return;
      }
    try {
      ensureHistoryStoreContext();
    ...
    await openConsigneHistoryEntryEditor(row, consigne, ctxRef, {
...
function bindConsigneRowValue(row, consigne, { onChange, initialValue } = {}) {
  ...
  const emit = (value) => {
    ...
    let valueForStatus = statusValue;
    try {
      const resolvedDayKey = normalizeHistoryDayKey(
        row?.dataset?.dayKey ||
          (typeof window !== "undefined" && window.AppCtx && typeof window.AppCtx.dateIso === "string"
            ? window.AppCtx.dateIso
            : ""),
      );
      if (consigne?.id != null && resolvedDayKey) {
        try {
          ensureHistoryStoreContext();
          const storeEntry = historyStoreGetEntry(consigne.id, resolvedDayKey);
          if (storeEntry && Object.prototype.hasOwnProperty.call(storeEntry, "value")) {
            valueForStatus = storeEntry.value;
          }
        } catch (_) {
          valueForStatus = statusValue;
        }
      }
    } catch (_) {}
    updateConsigneStatusUI(row, consigne, valueForStatus);
...
const getRecentResponsesStore = () => null;

const clearRecentResponsesForConsigne = () => {};

const removeRecentResponsesForDay = () => {};

async function deleteAllResponsesForDay() {
  return;
}
...
function syncDailyRowFromHistory(consigneId, dayKey, { entry, fallbackDayKey, silent = false } = {}) {
  ...
  ensureHistoryStoreContext();
  const normalized = normalizeHistoryDayKey(dayKey);
  const fallback = fallbackDayKey ? normalizeHistoryDayKey(fallbackDayKey) : "";
  const effectiveKey = normalized || fallback;
  const selector = effectiveKey
    ? `[data-consigne-id="${escapeSelectorToken(consigneId)}"][data-day-key="${escapeSelectorToken(effectiveKey)}"]`
    : `[data-consigne-id="${escapeSelectorToken(consigneId)}"]`;
  let dailyRow = null;
  try {
  ...
  const consigneShape = {
    id: consigneId,
    type: inferConsigneTypeFromRow(dailyRow, consigneId),
  };

  applyHistoryEntryToRow(consigneShape, effectiveKey || normalized || dayKey || dailyRow.dataset?.dayKey || "", record, {
    row: dailyRow,
    silent,
    fallbackDayKey: fallbackDayKey || normalized || dayKey || null,
  });

  const summaryData = normalizeSummaryMetadataInput(record);
  if (summaryData) {
    setConsigneSummaryMetadata(dailyRow, summaryData);
  } else {
    clearConsigneSummaryMetadata(dailyRow);
  }

  try {
    logConsigneSnapshot("daily.row.sync", consigneShape, {
      row: dailyRow,
      extra: {
...
if (typeof window !== "undefined" && !window.__historyEntryUpdatedBound) {
  try {
    window.__historyEntryUpdatedBound = true;
    window.addEventListener(
      HISTORY_EVENT_NAME,
      (event) => {
        const detail = event?.detail || {};
        const consigneId = detail.consigneId || detail.id;
        if (!consigneId) {
          return;
        }
        ...
        const targetDayKey = normalizedDayKey || detail.dayKey;
        syncDailyRowFromHistory(consigneId, targetDayKey, {
          entry,
        ...
        try {
          const entries = HistoryStore && typeof HistoryStore.getEntries === "function"
            ? HistoryStore.getEntries(consigneId)
            : [];
          const selector = `[data-consigne-id="${escapeSelectorToken(consigneId)}"]`;
    ...
    if (clearBtn) {
      ...
      clearBtn.addEventListener('click', async (event) => {
        ...
        try {
          ...
          applyDailyPrefillUpdate(consigne.id, scopeDayKey, "", { remove: true, silent: false });
          dispatchHistoryUpdateEvent({
            consigneId: consigne.id,
            dayKey: scopeDayKey,
            entry: null,
    ...
    form?.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (isRawEmpty && !note) {
          ...
          if (storeRecord) {
            applyDailyPrefillUpdate(consigne.id, scopeDayKey, canonicalValue, { entry: storeRecord, silent: false });
            dispatchHistoryUpdateEvent({
              consigneId: consigne.id,
              dayKey: scopeDayKey,
              entry: storeRecord,
...
async function renderDaily(ctx, root, opts = {}) {
  ...
  queueMicrotask(() => {
    lockedAutoSaveScopes.clear();
    isDailyHydrationLocked = false;
  });
  ...
  try {
    applyDailyPrefillUpdate = (consigneId, targetDayKey = dayKey, nextValue = "", extra = {}) => {
      ...
      const canonicalTargetKey = (() => {
        if (typeof opts.canonicalDayKey === "string" && opts.canonicalDayKey.trim()) {
          return opts.canonicalDayKey.trim();
        }
        if (typeof targetDayKey === "string" && targetDayKey.trim()) {
          return normalizeHistoryDayKey(targetDayKey.trim());
        }
        if (typeof dayKey === "string" && dayKey.trim()) {
          return normalizeHistoryDayKey(dayKey.trim());
        }
        return "";
      })();

      const ensureStoreContext = () => {
        const activeCtx = typeof window !== "undefined" ? window.AppCtx || null : null;
        if (HistoryStore && typeof HistoryStore.configure === "function") {
          HistoryStore.configure({
            db: activeCtx?.db || null,
            uid: activeCtx?.user?.uid || null,
            dayKeyFromDate: activeCtx?.Schema?.dayKeyFromDate || Schema?.dayKeyFromDate || null,
          });
        }
      };
      ...
      if (removeEntry) {
        try {
          ensureStoreContext();
          historyStoreRemove(consigneId, canonicalTargetKey || targetDayKey || "");
        } catch (_) {
  ...
  const renderItemCard = (item, { isChild = false, deferEditor = false, editorOptions = null } = {}) => {
    ...
    if (item?.id != null) {
      ...
      try {
        ...
        if (historyEntry && Object.prototype.hasOwnProperty.call(historyEntry, "value")) {
          ...
          initialFormValue = historyEntry.value;
        }
      } catch (_) {
        previousEntry = null;
    ...
    let initialValue = initialFormValue;
    try {
      const targetDayKey = normalizedCurrentDayKey || dayKey || row.dataset?.dayKey || fallbackDayKey || "";
      applyHistoryEntryToRow(item, targetDayKey, previousEntry, {
        row,
        silent: true,
        fallbackDayKey: fallbackDayKey || dayKey || normalizedCurrentDayKey || null,
      });
      try {
        initialValue = readConsigneCurrentValue(item, row);
      } catch (_) {
        initialValue = previousEntry?.value ?? null;
      }
    } catch (error) {
      modesLogger?.warn?.("daily.render.apply", {
        consigneId: item?.id ?? null,
        message: String(error?.message || error),
      });
      initialValue = previousEntry?.value ?? null;
  ...
  const renderGroup = (group, target) => {
    ...
    const childConfigs = group.children.map((child) => {
      ...
      const childRow = createHiddenConsigneRow(child, { initialValue: historyEntry?.value ?? null });
      childRow.dataset.parentId = child.parentId || group.consigne.id || "";
      childRow.draggable = false;
      parentCard.appendChild(childRow);
      ...
      let childInitialValue = historyEntry?.value ?? null;
      try {
        const targetDayKey = normalizedCurrentDayKey || dayKey || childRow.dataset?.dayKey || null;
        applyHistoryEntryToRow(child, targetDayKey, historyEntry, {
          row: childRow,
          silent: true,
          fallbackDayKey: dayKey || normalizedCurrentDayKey || null,
        });
        try {
          childInitialValue = readConsigneCurrentValue(child, childRow);
        } catch (_) {
          childInitialValue = historyEntry?.value ?? null;
        }
      } catch (error) {
        modesLogger?.warn?.("daily.render.child.apply", {
          consigneId: child?.id ?? null,
          parentId: group?.consigne?.id ?? null,
          message: String(error?.message || error),
        });
        childInitialValue = historyEntry?.value ?? null;
      }
