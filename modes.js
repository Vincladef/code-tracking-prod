...
function inputForType(consigne, initialValue = null, options = {}) {
  ...
  if (consigne.type === "checklist") {
    const items = sanitizeChecklistItems(consigne);
    const consigneId = consigne?.id != null ? String(consigne.id) : "";
    const stableItemIds = Array.isArray(consigne?.checklistItemIds) ? consigne.checklistItemIds : [];
      ...
      .map((label, index) => {
        ...
        return `
          <label class="flex items-center gap-2 rounded border border-slate-200 bg-slate-50 px-3 py-2 text-sm" data-checklist-item data-item-id="${escapeHtml(itemId)}" data-checklist-key="${escapeHtml(itemId)}" data-checklist-legacy-key="${escapeHtml(legacyId)}" data-checklist-index="${index}" data-checklist-label="${escapeHtml(trimmedLabel)}" data-validated="false">
            <input type="checkbox" class="h-4 w-4" data-checklist-input data-key="${escapeHtml(itemId)}" data-checklist-key="${escapeHtml(itemId)}" data-legacy-key="${escapeHtml(legacyId)}" data-checklist-index="${index}">
            <span class="flex-1">${escapeHtml(label)}</span>
            <button type="button" class="checklist-skip-btn" data-checklist-skip-btn aria-pressed="false" title="Passer cet élément (ne pas le compter)">⏭</button>
          </label>`;
      })
      .join("");
    const hiddenValue = escapeHtml(JSON.stringify({ items: [], skipped: [] }));
    const rootConsigneAttr = consigneId ? ` data-consigne-id="${escapeHtml(consigneId)}"` : "";
    const scriptContent = String.raw`
      <script>
        (() => {
          ...
          const resolveHost = (input) => (input && typeof input.closest === 'function' ? input.closest('[data-checklist-item]') : null);
          ...
          const updateHidden = () => {
            ...
            const payload = {
              ...
              skipped: inputs.map((input) => Boolean(input?.dataset && input.dataset.checklistSkip === '1')),
            };
            try {
          ...
          root.addEventListener('change', (event) => {
            const target = event.target instanceof Element ? event.target.closest('[data-checklist-input]') : null;
            ...
            if (target.dataset) {
              delete target.dataset.checklistSkip;
            }
            target.removeAttribute('data-checklist-skip');
            target.disabled = false;
            updateHidden();
          });
          root.addEventListener('click', (event) => {
            const button = event.target instanceof Element ? event.target.closest('[data-checklist-skip-btn]') : null;
            if (!button) return;
            event.preventDefault();
    ...
    `;
    return `
      <div class="space-y-2" data-checklist-root${rootConsigneAttr}>
        <input type="hidden" name="checklist:${consigne.id}" value="${hiddenValue}" data-checklist-state data-autosave-track="1">
        ${checkboxes}
      </div>
      ${scriptContent}
    `;
...
// even when the DOM timeline is not yet initialised or is off-screen (e.g., bilan pages).
...
async function openBilanHistoryEditor(row, consigne, ctx, options = {}) {
  ...
  // Enhance textarea in both rendering modes
...
async function openConsigneHistoryEntryEditor(row, consigne, ctx, options = {}) {
  ...
  if (consigne.type === "checklist") {
    try {
      // Checklist scope initialisation retirée (hydratation supprimée)
    } catch (_) {}
  }
  try {
