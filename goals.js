...
export function openGoalForm(ctx, goal=null){
  D.group("goals.openGoalForm", { isEdit: !!goal });
...
async function saveGoalResponse(ctx, goal, value){
  D.group("goals.saveGoalResponse", { goalId: goal.id, value });
  const payload = {
...
export async function openGoalTracker(ctx, goal){
  D.group("goals.openGoalTracker", goal.id);
  const root = $("#view-root");
  root.innerHTML = "";
